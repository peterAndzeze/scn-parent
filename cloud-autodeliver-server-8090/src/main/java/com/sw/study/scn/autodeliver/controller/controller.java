package com.sw.study.scn.autodeliver.controller;
...
public class controller {
    ...
    public String findResumeOpenState(@PathVariable Long userId) {
        ...
        String forObject = restTemplate.getForObject("http://localhost:8080/resume/openstate/" + userId, String.class);
        return forObject;
    }
    ...
    public Integer findResumeOpenState(@PathVariable Long userId) {
        ...
        // 1、从 Eureka Server中获取resume-server服务的实例信息（使用客户端对象做这件事）
        List<ServiceInstance> instances = discoveryClient.getInstances("resume-server");
    ...
    public String findResumeOpenStateRibbon(@PathVariable Long userId) {
        // 使用ribbon不需要我们自己获取服务实例然后选择一个那么去访问了（自己的负载均衡）
        String url = "http://resume-server/resume/openstate/" + userId;  // 指定服务名
    ...
    public String findResumeOpenStateTimeoutFallback(@PathVariable Long userId) {
        // 使用ribbon不需要我们自己获取服务实例然后选择一个那么去访问了（自己的负载均衡）
        String url = "http://resume-server/resume/openstate/" + userId;  // 指定服务名
        String forObject = restTemplate.getForObject(url, String.class);
        return forObject;
